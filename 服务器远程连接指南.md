...
ssh wubug-server
# 或直接使用IP连接
ssh -i ~/.ssh/id_rsa_wubug root@112.74.163.102
```
...
# 或使用PuTTY等工具
putty.exe -ssh root@112.74.163.102 -i ~/.ssh/id_rsa_wubug.ppk
```
...
# 上传到服务器
scp -i ~/.ssh/id_rsa_wubug -r dist/* root@112.74.163.102:/var/www/html/
```

### 后端部署
```bash
# 上传代码
scp -i ~/.ssh/id_rsa_wubug -r src/ root@112.74.163.102:/var/www/douyin-admin-master/
...
# 检查密钥权限
ls -la ~/.ssh/id_rsa_wubug
chmod 600 ~/.ssh/id_rsa_wubug
```
...
- **2025-12-04**: 移除老服务器配置，统一使用新服务器 (112.74.163.102)
